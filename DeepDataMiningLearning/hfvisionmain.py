...
import sys
sys.path.append("/content/drive/MyDrive/DeepDataMiningLearning-main")
...
output_dir="/content/drive/MyDrive/258_hw4/output"
traintag="hw3_1"
data_path="/content/drive/MyDrive/258_hw4/cached"
...
#parser.add_argument('--task', type=str, default="object-detection",help='tasks: image-classification, object-detection')

data_name="keremberke/football-object-detection"#"detection-datasets/coco" #"tiny_coco"
#help='data name: detection-datasets/coco, food101, beans, cats_vs_dogs,cppe-5')

datasplit='train'
#help='dataset split name in huggingface dataset')

datatype='huggingface'
#help='Data type: huggingface, torch')

format='coco'
#help='dataset bbox format: pascal_voc, coco')

train_dir="/content/drive/MyDrive/258_hw4/small_coco"#/train_2017_small"
label_path="/content/drive/MyDrive/258_hw4/small_coco/annotations/instances_train2017_small.json"
...
model_name_or_path="facebook/detr-resnet-50"#"emre/detr-resnet-50_finetuned_cppe5"#
...
def load_visiondataset(data_name=None, split="train", train_dir=None, validation_dir=None, task="image-classification", format='coco', max_train_samples = 2000, train_val_split=0.15, \
    ...
    print("num1",image_column_name,dataset_column_names)
    print("num2",label_column_name,dataset_column_names)
    ...
    if task == "image-classification":
        ...
        dataset_objectdetection_select(split_datasets["train"], data_index=0, id2label=id2label, categories=categories, format=format, \
                                       image_column_name="image", label_column_name=label_column_name, output_folder="output/")
...
def custom_train(model, image_processor, train_dataloader, eval_dataloader, metriceval, device, accelerator=None, do_evaluate=False):
     # Optimizer
...
def trainmain():
    ...
    output_dir="/content/drive/MyDrive/258_hw4/output"
    trainoutput=os.path.join(output_dir, "hw3")#data_name+'_'+traintag)
    ...
    if accelerator.is_main_process:
        if os.environ.get('HF_HOME') is not None:
            ...
            #mycache_dir = deviceenv_set(args.usehpc, data_path)
            os.environ['HF_HOME'] = data_path
            mycache_dir = data_path
        else:
            mycache_dir = '~/.cache/huggingface/'
        print("Cache dir:", mycache_dir)
        device, useamp = get_device(gpuid=gpuid, useamp=True)
    ...
    with accelerator.main_process_first():
        dataset, labels, id2label, image_column_name, label_column_name = load_visiondataset(data_name=data_name, \
                                    split=datasplit, train_dir=train_dir, validation_dir=validation_dir, \
                                    task=task, format=format, max_train_samples = max_train_samples, train_val_split=train_val_split, \
                                    image_column_name="image", label_column_name="labels", mycache_dir=mycache_dir)

    # Load pretrained model and image processor
    #
    # In distributed training, the .from_pretrained methods guarantee that only one local process can concurrently
    # download model & vocab.
    model, image_processor = load_visionmodel(model_name_or_path, task=task, load_only=False, labels=labels, mycache_dir=mycache_dir, trust_remote_code=True)
    ...
    with accelerator.main_process_first():
        preprocess_train, preprocess_val = dataset_preprocessing(image_processor=image_processor, task=task, size=size, format=format, image_column_name=image_column_name, label_column_name=label_column_name)
        ...
        if task == "image-classification":
            ...
            coco_datafolder = os.path.join(mycache_dir, 'coco_converted', data_name)
            eval_dataset = HFCOCODataset(dataset[valkey], id2label, dataset_folder=coco_datafolder, coco_anno_json=None, data_type=datatype, format=format, image_processor=image_processor)
    ...
    collate_fn = get_collate_fn(task, image_processor, label_column_name)

    metriceval = myEvaluator(task=task, useHFevaluator=True, dualevaluator=False, \
    ...
    if trainmode == 'HFTrainer':
        training_args = TrainingArguments(
            ...
            warmup_steps=num_warmup_steps, #500,
            logging_steps=100,
        ...
        #checkpoint = load_hfcheckpoint(resume_from_checkpoint)
        ...
        train_dataloader = DataLoader(
            train_dataset, shuffle=True, collate_fn=collate_fn, batch_size=per_device_train_batch_size)
        eval_dataloader = DataLoader(eval_dataset, shuffle=False, collate_fn=collate_fn, batch_size=per_device_eval_batch_size)
        evaluate_dataset(model, eval_dataloader, task, metriceval, device, image_processor=image_processor, accelerator=accelerator)
        ...
        if trainmode == 'CustomTrain':
            custom_train(model, image_processor, train_dataloader, eval_dataloader, metriceval, device, accelerator, do_evaluate=False)
        else:
            evaluate_dataset(model, eval_dataloader, task, metriceval, device, image_processor=image_processor, accelerator=accelerator)
